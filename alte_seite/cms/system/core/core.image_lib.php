...
class Image_lib {
    ...
    function Image_lib()
    {
		...
		$LANG->fetch_language_file('image_lib');				
    }
    /* END */
    ...
    { 
		...
		$this->full_src_path = $FNS->remove_double_slashes($this->file_path.'/'.$this->file_name);    	
		$this->full_dst_path = $FNS->remove_double_slashes($this->file_path.'/'.$xp['name'].$this->thumb_prefix.$xp['ext']);
		...
		if ($this->wm_font != '')
		{
			$this->wm_font = PATH.'/fonts/'.$this->wm_font;
		}
	...
	function image_rotate()
	{
		...
		if ($this->resize_protocol == 'imagemagick' || $this->resize_protocol == 'netpbm')
		{
			$protocol = 'image_process_'.$this->resize_protocol;
 		...
 		if ($this->rotation == 'hor' || $this->rotation == 'vrt')
 		{
    ...
    */
	...
	function set_error($str)
	{
		...
		if (is_array($str))
		{
			foreach ($str as $val)
			{
				$this->error_msg[] = $LANG->line($val);
			}		
		}
		else
		{
			$this->error_msg[] = $LANG->line($str);
		}
	}
	/* END */
	...
	function show_error()
	{
		...
		return $DSP->error_message($this->error_msg);
	}
	/* END */
