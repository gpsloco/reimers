...
class Functions {  
    ...
    */
	...
	function prep_conditionals($str, $vars, $safety='n', $prefix='')
	{
		...
		$vars = array_merge($vars, $TMPL->embed_vars);
		...
		$str = str_replace(LD.'if:else'.RD, $this->unique_marker('if_else_safety'), $str);
		...
		$str = str_replace($this->unique_marker('if_else_safety'), LD.'if:else'.RD, $str);
	...
	function unique_marker($ident)
	{
		...
		if ( ! $rand)
		{
			...
			for ($i=0; $i < 32; $i++)
			{
				$rand .= substr($pool, mt_rand(0, strlen($pool) -1), 1);
			}			
		}
