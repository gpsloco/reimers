	...
	function decode($array=FALSE)
	{
		...
		if ($array !== FALSE && is_array($array))
		{
			while (list($key) = each($array))
			{
				...
				{
					$array[$key] = ($this->xss_clean) ? $REGX->xss_clean($array[$key]) : $array[$key];
				}
			}
		...
		else
		{
			...
			{
				$result = ($this->xss_clean) ? $REGX->xss_clean($result) : $result;
			}
		}
...
class XML_RPC_Message extends XML_RPC
{
	...
	function closing_tag($the_parser, $name)
	{
		...
		switch($name)
		{
			...
			break;
			case 'METHODNAME':
				$this->xh[$the_parser]['method'] = preg_replace("#^[\n\r\t ]+#", '', $this->xh[$the_parser]['ac']);
			break;
			case 'PARAMS':
			case 'FAULT':
			case 'METHODCALL':
	...
	function output_parameters($array=FALSE)
	{
		...
		if ($array !== FALSE && is_array($array))
		{
			while (list($key) = each($array))
			{
				...
				else
				{
					...
					$array[$key] = ($key == 'bits' OR $this->xss_clean == FALSE) ? $array[$key] : $REGX->xss_clean($array[$key]);
				}
			}
		...
		else
		{
			...
			for ($i = 0; $i < sizeof($this->params); $i++)
    		{
    			...
    			{
    				$parameters[] = ($this->xss_clean) ? $REGX->xss_clean($a_param) : $a_param;
    			}
    		}	
    	}
