...
Class Magpie {
    ...
    function Magpie()
    {
    	global $TMPL, $LOC, $FNS, $PREFS, $SESS;
    	
    	/** -------------------------------
    	/**  Set Parameters 
    	/** -------------------------------*/
    	
    	$this->cache_refresh 	= ( ! $TMPL->fetch_param('refresh')) ? $this->cache_refresh : $TMPL->fetch_param('refresh');
    	$this->page_url 		= ( ! $TMPL->fetch_param('url')) ? '' : str_replace('&#47;', '/',trim($TMPL->fetch_param('url')));
    	$limit					= ( ! $TMPL->fetch_param('limit')) ? 20 : $TMPL->fetch_param('limit');
    	...
    	if ($PREFS->ini('debug') == 2 OR ($PREFS->ini('debug') == 1 && $SESS->userdata['group_id'] == 1))
		{
        ...
        {
        	define('MAGPIE_CACHE_DIR', PATH_CACHE.$this->cache_name.'/');
        }
		...
		if (preg_match("/(".LD."items".RD."(.*?)".LD.SLASH.'items'.RD."|".LD."magpie:items".RD."(.*?)".LD.SLASH.'magpie:items'.RD.")/s", $template, $matches))
        {
        	$items_data = '';
        	$i = 0;
        	...
        	{        	
        		foreach($this->RSS->items as $item)
        		{
					...
					{
						$tagdata = $FNS->prep_conditionals($temp_data, $item, '', 'magpie:');
        			}
			...
			$items_data = str_replace(LD.SLASH.'exp:', 'Mu87ddk2QPoid990iod', $items_data);
		
			$items_data = preg_replace("/".LD."if.*?".RD.".+?".LD.SLASH."if".RD."/s", '', $items_data);
			$items_data = preg_replace("/".LD.".+?".RD."/", '', $items_data);

			$items_data = str_replace('TgB903He0mnv3dd098', LD.'exp:', $items_data);
			$items_data = str_replace('Mu87ddk2QPoid990iod', LD.SLASH.'exp:', $items_data);
        ...
        {        			
			...
			if ($key == "feed_version" OR $key == "magpie:feed_version")
			{
				...
				$template = $TMPL->swap_var_single($val, $this->RSS->feed_version, $template);
			}
			...
			if (($key == "feed_type" OR $key == "magpie:feed_type") && isset($this->RSS->feed_type))
			{
				...
				$template = $TMPL->swap_var_single($val, $this->RSS->feed_type, $template);
			}
			...
			foreach ($image_variables as $variable)
			{
				if ($key == 'image_'.$variable OR $key == 'magpie:image_'.$variable)
				{
					...
					$template = $TMPL->swap_var_single($val, $this->RSS->image[$variable], $template);
				}
			}
			...
			foreach ($channel_variables as $variable)
			{
				if ($key == 'channel_'.$variable OR $key == 'magpie:channel_'.$variable)
				{
					...
					$template = $TMPL->swap_var_single($val, $this->RSS->channel[$variable], $template);
				}
			}			
			...
			{
				$template = $TMPL->swap_var_single($val, $this->page_url, $template);
			}	
		}
...
\*======================================================================*/
    ...
    */
    function create_parser($source, $out_enc, $in_enc, $detect)
    {
        ...
        $this->encoding = $PREFS->ini('charset');
...
\*=======================================================================*/		
	function file_name ($url)
	{
		...
		$filename = md5( $url );
		return $FNS->remove_double_slashes(join( DIRECTORY_SEPARATOR, array( $this->BASE_CACHE, $filename)));
	}
	...
	function fetch($URI)
	{
		...
		switch(strtolower($URI_PARTS["scheme"]))
		{
			case "http":
				...
				if($this->_connect($fp))
				{
					// We needed port 80 to connect, but we can now switch to the proper one
