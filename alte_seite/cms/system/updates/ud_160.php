...
=====================================================
 File: ud_160.php
-----------------------------------------------------
 Purpose: Performs version 1.6.0 update
=====================================================
*/
...
class Updater {
	...
	function do_update()
	{
		...
		{
			$prefs['theme_folder_path'] = preg_replace("#/+#", "/", substr(str_replace("\\", "/", realpath(PATH)), 0, - strlen($UD->ini('system_folder').'/')).'/themes/');
		}
		else
		{
		...
		{
			$prefs[$value] = str_replace('\\', '\\\\', $UD->ini($value));
		}
		...
		*/
		...
		$query = $DB->query("SELECT cat_id, group_id FROM exp_categories");
		...
		if ($query->num_rows > 0)
		{
			...
			{
				$DB->query($DB->insert_string('exp_category_field_data', $cat));
			}
		}
		...
		$conf['doc_url'] = str_replace('eedocs.pmachine.com', 'expressionengine.com/docs', $conf['doc_url']);
	...
	function create_short_name($str)
	{
		...
		elseif(function_exists('iconv') AND ($iconvstr = @iconv('', strtoupper($UD->ini('charset')), $str)) !== FALSE)
		{
			$str = $iconvstr;
		}
		else
