...
=====================================================
 File: ud_099.php
-----------------------------------------------------
 Purpose: Perform version public beta 2 update
=====================================================
*/
...
class Updater {
	...
	function do_update()
	{
		...
		$DB->query("insert into exp_template_groups(group_id, group_name, group_order) values ('', 'search', '$num')");
		
		$id = $DB->insert_id;
		
		$Q[] = "insert into exp_templates(template_id, group_id, template_name, template_data) values ('', '$id', 'index', '".addslashes(search_index())."')";
		$Q[] = "insert into exp_templates(template_id, group_id, template_name, template_data) values ('', '$id', 'results', '".addslashes(search_results())."')";
		$Q[] = "insert into exp_templates(template_id, group_id, template_name, template_type, template_data) values ('', '$id', 'search_css', 'css', '".addslashes(search_css())."')";
		...
		$query = $DB->query("SELECT group_id FROM exp_member_groups ORDER BY group_id");

		foreach ($query->result as $row)
		{
			$flood = ($row['group_id'] == 1) ? '0' : '30';
		
			$DB->query("UPDATE exp_member_groups SET can_search = 'y', search_flood_control = '$flood' WHERE group_id = '".$row['group_id']."'");
		
			$st = ($row['group_id'] == 1) ? 'y' : 'n';
		
			$DB->query("UPDATE exp_member_groups SET can_moderate_comments = '$st' WHERE group_id = '".$row['group_id']."'");
		}
		...
		foreach ($query->result as $row)
		{
			...
			$res = $DB->query("SELECT member_id FROM exp_member_homepage WHERE member_id = '$member_id'");
			
			if ($res->num_rows == 0)
				$DB->query("INSERT INTO exp_member_homepage (member_id) VALUES ('$member_id')");
	
			if ($md_exists == TRUE)
			{
				$res = $DB->query("SELECT member_id FROM exp_member_data WHERE member_id = '$member_id'");
	
				if ($res->num_rows == 0)
				{
					$DB->query("INSERT INTO exp_member_data (member_id) VALUES ('$member_id')");
				}
			}
		}
