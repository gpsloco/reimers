...
class Wiki {
	...
	function Wiki($return = FALSE)
	{
		...
		// stripslashes is a temp fix to ditch slashes @todo
		$this->seg_parts = explode('/', $REGX->xss_clean(strip_tags(stripslashes($IN->QSTR))));
	...
	{	
		...
		if (stristr($title, ':') && sizeof($this->namespaces) > 0)
		{
			...
			foreach($this->namespaces as $name => $label)
			{
				...
				{
					$xsql		 = " AND LOWER(page_namespace) = '".$DB->escape_str($name)."' ";
		...
		return $DB->query("SELECT * FROM exp_wiki_page 
						   ...
						   AND LOWER(page_name) = '".$DB->escape_str(strtolower($this->topic))."' 
						   {$xsql}");
	}
	/* END */
	...
	function title_list($type = '')
	{
		...
		{
			$xsql .= "AND LOWER(p.page_namespace) = '".$DB->escape_str(strtolower($this->seg_parts['1']))."'";
		}
		else
		{
   ...
   /* END */
		...
		else
		{
			...
			{	
				...
				if ($IN->GBL('rename') !== FALSE && $IN->GBL('rename') != '')
				{
					...
					if (stristr($IN->GBL('rename'), ':') && sizeof($this->namespaces) > 0)
					{
						...
						foreach($this->namespaces as $name => $label)
						{
							if ($label['1'] == $parts['0'])
							{
								...
								$data['page_name']		 = $this->valid_title(substr($IN->GBL('rename'), strlen($label['1'].':')));
					...
					$t_query = $DB->query("SELECT COUNT(*) AS count FROM exp_wiki_page WHERE page_name = '".$DB->escape_str($data['page_name'])."' AND LOWER(page_namespace) = '".$DB->escape_str($data['page_namespace'])."'");
    ...
    function convert_curly_brackets($str)
	{
    	...
    	$protected = array();
    	$front_protect = $FNS->unique_marker('wiki_front_protect');
    	$back_protect  = $FNS->unique_marker('wiki_back_protect');
			...
			if (sizeof($regular) > 0)
			{
				...
				{	
					...
					$query = $DB->query("SELECT wn.namespace_label, wp.page_name
										...
										AND
										(
											LOWER(CONCAT_WS(':', REPLACE(wn.namespace_label, ' ', '{$replace}'), wp.page_name)) IN 
											('".strtolower(implode("','", $DB->escape_str($regular)))."')
											OR
											LOWER(wp.page_name) IN ('".strtolower(implode("','", $DB->escape_str($regular)))."')
										)
										");
