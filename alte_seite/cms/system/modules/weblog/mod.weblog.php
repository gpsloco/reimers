...
class Weblog {
           ...
           foreach ($TMPL->var_pair as $key => $val)
            ...
            {    
                ...
                if (isset($this->cfields[$row['site_id']][$val]))
                {
					...
					{
	                    $tagdata = $TMPL->swap_var_single($val, str_replace(array('{', '}'), array($FNS->unique_marker('channel_bracket_open'), $FNS->unique_marker('channel_bracket_close')), $entry), $tagdata);
					}
					else
					{
			...
			if (strpos($tagdata, $FNS->unique_marker('channel_bracket_open')) !== FALSE)
			{
				$tagdata = str_replace(array($FNS->unique_marker('channel_bracket_open'), $FNS->unique_marker('channel_bracket_close')), array('{', '}'), $tagdata);
			}
	...
	function next_prev_entry($which = 'next')
	{
		...
		if ( ! class_exists('Typography'))
		{
			require PATH_CORE . 'core.typography.php';
		}
